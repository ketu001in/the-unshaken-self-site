        ...
        <div className="w-[320px] sm:w-[380px] h-[500px] rounded-2xl glassmorphism border border-border-custom shadow-2xl flex flex-col overflow-hidden animate-[slideUp_0.3s_ease-out]">
          ...
          <div className="p-4 bg-gradient-to-r from-[#1e3f20]/95 to-[#122815]/95 text-stone-100 flex items-center justify-between border-b border-border-custom">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <span className="text-[12px] text-stone-400 font-light">The Unshaken Self Assistant</span>
              </div>
            </div>
            <button
          ...
          <div className="flex border-b border-border-custom bg-stone-50 dark:bg-[#080d0b]">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-[12px] uppercase tracking-widest font-bold cursor-pointer transition-colors ${
                viewMode === "chat"
          ...
          {viewMode === "chat" ? (
            <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-[#faf8f5]/65 dark:bg-[#070b09]/80 no-scrollbar">
              {messages.map((msg) => (
                ...
                >
                  <div
                    className={`max-w-[85%] rounded-2xl p-3 text-xs md:text-[15px] leading-relaxed shadow-sm ${
                      msg.sender === "user"
                  ...
                  >
                    ...
                    {isOpenItem && (
                      <div className="px-3 pb-3 space-y-2.5">
                        <p className="text-[13px] md:text-xs text-stone-600 dark:text-stone-300 leading-relaxed">
                          {item.a.replace(CONTACT_EMAIL_PLACEHOLDER, contactEmail)}
                        </p>
                        <a
                          href={`mailto:${contactEmail}`}
                          className="inline-flex items-center gap-1.5 text-[12px] font-semibold text-[#dfb15b] hover:underline"
                        >
            ...
            <div className="px-4 py-2 bg-stone-50 dark:bg-[#080d0b] border-t border-border-custom overflow-x-auto whitespace-nowrap flex space-x-2 no-scrollbar">
              {suggestions.map((s, idx) => (
                <button
                  ...
                  className="px-2.5 py-1 rounded-full bg-white dark:bg-[#121c19] text-foreground hover:bg-[#1e3f20]/5 dark:hover:bg-[#dfb15b]/5 border border-border-custom text-[12px] cursor-pointer transition-colors"
                >
                  {s}
                </button>
              ))}
